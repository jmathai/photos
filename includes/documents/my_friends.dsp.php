<?php
  ...
  $friends    = $u->getFriends($user_id, 'Confirmed');
  $myFriends  = $u->getFriends($_USER_ID, 'Confirmed');
  $activity   = $u->getFriendActivitiesFull($user_id, 30);
  ...
  <?php
    if(count($friends) > 0)
    {
      foreach($friends as $v)
      {
        ...
        echo '<div>
                <a href="/users/' . $v['U_USERNAME'] . '/" class="plain"><img src="' . $avatarSrc . '" width="30" height="30" hspace="10" vspace="2" border="0" align="absmiddle" />' . $v['U_USERNAME'] . '</a>
                <br clear="all" />';
        if($logged_in === true)
        {
          if(!isset($friendIds[$v['U_USERNAME']]) && $_USER_ID != $v['U_ID'])
          {
            $randId = 'addFriend_' . rand(0, 1000);
            echo '<a href="javascript:void(0);" onclick="inviteFriendForm(\'' . $comment['A_EXTRA_2'] . '\', $(\'' . $randId . '\'));" id="' . $randId . '" class="plain" title="add ' . htmlspecialchars($comment['A_EXTRA_2']) . ' as a friend"><img src="/images/icons/user_add_16x16.png" class="png" width="16" height="16" border="0" hspace="5" vspace="5" /></a>';
          }
        }
        echo '  <a href="javascript:void(0);" onclick="messageForm(\'' . $v['U_USERNAME'] . '\', this);" title="send ' . $v['U_USERNAME'] . ' a message"><img src="/images/icons/chat_bubble_alt_2_16x16.png" class="png" width="16" height="16" border="0" hspace="5" vspace="5" /></a>
