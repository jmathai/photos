<?php
  ...
  if((count($elementsArr) > 0) && ($slide < count($elementsArr)))
  {
    ...
    if($prev >= 0)
    {
      ...
      {
        echo '<div style="float:left; padding-right:5px;"><a href="/?action=flix.html_slideshow&key=' . $key . '&slide=' . $prev . '"><img src="' . PATH_FOTO . $elementsArr[$prev]['thumbnailPath_str'] . '" border="0" width="75" height="75" /></a></div>';
      }
      else if(array_key_exists('title_str', $elementsArr[$prev]))
      {
        echo '<a style="cursor:pointer;" href="/?action=flix.html_slideshow&key=' . $key . '&slide=' . $prev . '" class="plain"><div style="float:left; padding-right:5px; width:75px; height:75px; background-color:#' . substr($elementsArr[$prev]['backgroundColor_str'], 2). ';"><span style="font-size:8pt; color:#' . substr($elementsArr[$prev]['mainColor_str'], 2) . ';">' . $elementsArr[$prev]['title_str'] . '</span></div></a>';
      }
      else 
      {
        echo '<a style="cursor:pointer;" href="/?action=flix.html_slideshow&key=' . $key . '&slide=' . $prev . '" class="plain"><div style="float:left; padding-right:5px; width:75px; height:75px; background-color:#FFFFFF;">Preview not available</div></a>';
      }
    }
    else 
    {
    ...
    if($next < count($elementsArr))
    {
      ...
      {
        echo '<div><a href="/?action=flix.html_slideshow&key=' . $key . '&slide=' . $next . '"><img src="' . PATH_FOTO . $elementsArr[$next]['thumbnailPath_str'] . '" border="0" width="75" height="75" /></a></div>';
      }
      else if(array_key_exists('title_str', $elementsArr[$next]))
      {
        echo '<a style="cursor:pointer;" href="/?action=flix.html_slideshow&key=' . $key . '&slide=' . $next . '" class="plain"><div style="width:75px; height:75px; background-color:#' . substr($elementsArr[$next]['backgroundColor_str'], 2). ';"><span style="font-size:8pt; color:#' . substr($elementsArr[$next]['mainColor_str'], 2) . ';">' . $elementsArr[$next]['title_str'] . '</span></div></a>';
      }
      else 
      {
        echo '<a style="cursor:pointer;" href="/?action=flix.html_slideshow&key=' . $key . '&slide=' . $next . '" class="plain"><div style="width:75px; height:75px; background-color:#FFFFFF;">Preview not available</div></a>';
      }
    }
    else 
    {
